...
internal class TorControlParser {

    sealed class ParseState {
        abstract fun parseLine(line: String): Pair<ParseState, TorControlResponse?>
        ...
        object AwaitingReply : ParseState() {
            override fun parseLine(line: String): Pair<ParseState, TorControlResponse?> {
        ...
        data class BuildingReply(val status: Int, val replies: List<TorControlResponse.Reply>) : ParseState() {
            override fun parseLine(line: String): Pair<ParseState, TorControlResponse?> {
                val (status, type, reply) = parseReplyLine(line)
                if (status != this.status) println("Got additional reply line with different status (line: $status, original: ${this.status})")
    ...
    fun parseLine(line: String): TorControlResponse? {
        val (nextState, response) = state.parseLine(line)
        state = nextState
        return response
    }
